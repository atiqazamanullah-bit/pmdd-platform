...
class OrchestratorAgent(BaseCognitiveAgent):
    """Agent 5: Master Orchestrator."""
    
    def __init__(self):
        super().__init__(agent_name="Agent5_Orchestrator", max_retries=3)
    ...
    async def observe(self, state: CognitiveState) -> Dict[str, Any]:
        return {"memory": "Synthesis context loaded."}

    async def analyze(self, context: Dict[str, Any], state: CognitiveState) -> AgentOutput:
        prompt = ChatPromptTemplate.from_messages([
            ("system", ORCHESTRATOR_SYSTEM_PROMPT),
            ("human", "Previous Agent Findings:\n{findings}\n\nOriginal Segment:\n{text}\n\nSynthesize the above findings into a final, highly rigorous output.")
        ])
        
        # We expect context to contain the findings from agents 2, 3, 4
        chain = prompt | self.structured_llm
        result = await chain.ainvoke({
            "findings": json.dumps(context.get("previous_findings", []), default=lambda x: x.dict()),
    ...
    async def reflect_and_correct(self, context: Dict[str, Any], state: CognitiveState) -> AgentOutput:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", ORCHESTRATOR_SYSTEM_PROMPT),
            ("human", "Original Segment:\n{text}\n\nYOUR PREVIOUS SYNTHESIS FAILED. Feedback: {feedback}\nCorrect your analysis and ensure perfect evidence tracing.")
        ])
        chain = prompt | self.structured_llm
